	...
	public function content( $results ) {
		...
		foreach ( $results as $key => $post ) {
			...
			if ( 'jnews_single_image_owl' === $image_mechanism ) {
				$image = \JNews\Image\ImageNormalLoad::getInstance()->owl_single_image( $post_thumbnail_id, 'jnews-360x504' );
			} else {
				$image = apply_filters( $image_mechanism, $post_thumbnail_id, 'jnews-360x504' );
			}

			$content .=
				'<div ' . jnews_post_class( 'jeg_slide_item', $post->ID ) . '>
                    ' . jnews_edit_post( $post->ID ) . '
                    <a href="' . get_the_permalink( $post ) . '" aria-label="Swipe article: ' . get_the_title( $post ) . "\">
                        {$image}
                    </a>
		...
		if ( ! empty( $result ) ) {
			...
			$output =
				"<div {$this->element_id($attr)} class=\"jeg_slider_wrapper jeg_slider_type_3_wrapper {$column_class} {$this->unique_id} {$this->get_vc_class_name()} {$attr['el_class']}\">
                    <div class=\"jeg_slider_type_3 jeg_slider\" data-items=\"{$number_item}\" data-autoplay=\"{$attr['enable_autoplay']}\" data-delay=\"{$autoplay_delay}\">
                        {$content}
                    </div>
                </div>";
